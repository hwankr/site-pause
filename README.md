...
방해되는 사이트·특정 페이지·쇼츠·릴스를 차단하는 **Chrome 확장 프로그램**입니다.
...
**사이트 관리 → 쇼츠·릴스 차단**에서 **YouTube 쇼츠 차단**, **Instagram 릴스 차단**을 선택하고 **저장 → 차단 켜기**를 누르세요. 주소를 따로 등록하지 않아도 사용할 수 있으며, 두 옵션은 처음에는 꺼져 있습니다.

- **YouTube:** `/shorts`와 그 아래 쇼츠 화면을 차단합니다. 일반 영상·검색·구독은 계속 이용할 수 있습니다.
- **Instagram:** `/reel`, `/reels`와 프로필의 릴스 탭을 차단합니다. 일반 게시물·프로필·DM은 계속 이용할 수 있습니다.
- **메뉴·추천 숨김:** 식별 가능한 쇼츠·릴스 메뉴와 추천 카드도 숨깁니다. 스크롤로 추가된 항목에도 적용하며, 차단을 끄면 숨겼던 항목을 복원합니다.
- **허용 예외 우선:** 허용한 페이지와 링크는 숨기거나 차단하지 않습니다. 사이트 전체를 허용하면 해당 사이트의 쇼츠·릴스 차단도 예외가 됩니다. 반대로 사이트 전체 차단 규칙이 있으면 일반 영상·게시물도 차단되므로, 쇼츠·릴스만 막으려면 해당 전체 차단 규칙을 지우세요.

직접 접속·사이트 내부 화면 이동·이미 열려 있는 대상 탭에 적용됩니다. 업데이트 직후 메뉴 숨김을 적용하려면 이미 열려 있던 YouTube·Instagram 탭도 한 번 새로고침하세요.

전용 화면과 식별 가능한 추천 영역을 기준으로 동작합니다. 쇼츠가 일반 영상 주소로 열리거나 릴스가 일반 피드에 섞여 표시되는 경우 모두 구분하지는 못하며, 사이트 구조가 바뀌면 숨김 기능 수정이 필요할 수 있습니다. 휴대폰 앱에는 적용되지 않습니다.

## 사이트별 사용 시간

팝업에서 **오늘의 총 사용 시간과 많이 사용한 사이트**를 확인할 수 있습니다. **자세히 보기**를 누르거나 사이트 관리 상단의 **사용 시간**을 누르면 **오늘 / 최근 7일 / 최근 30일**의 사이트별 순위와 날짜별 그래프가 열립니다.

상세 화면의 **전체 사이트 / 차단 목록 사이트** 필터로 현재 차단 목록에 등록한 사이트의 사용 시간만 확인할 수 있습니다. 필터는 총 사용 시간, 사이트별 순위, 날짜별 그래프에 함께 적용됩니다.
...
- **5분 초과부터 표시:** 사이트별 하루 누적 사용 시간이 5분 이하이면 통계에서 제외합니다. 같은 날 3분씩 두 번 사용하면 총 6분 전체가 표시됩니다. 최근 7일·30일 화면에서도 날짜별로 이 기준을 적용하므로, 어제 4분과 오늘 4분을 합쳐 표시하지 않습니다. 나중에 5분을 넘었는지 판단하기 위해 짧은 사용 시간도 브라우저 내부에는 누적하며, 기존 기록도 삭제하지 않습니다.
- **차단 목록 사이트 필터:** 현재 저장된 차단 목록을 기준으로 하며 차단 켜짐·꺼짐이나 허용 예외와 관계없이 포함합니다. 사이트 전체 차단은 하위 도메인까지 포함합니다. 특정 페이지 차단은 해당 페이지가 속한 호스트의 전체 사용 시간을 보여줍니다. 페이지 경로를 저장하지 않으므로 그 페이지만 사용한 시간을 따로 구분하지 않습니다.
...
- **등록 한도:** 차단·허용 주소를 합쳐 최대 200개까지 등록할 수 있습니다. 쇼츠·릴스 옵션은 이 한도와 별개입니다. 기존에 저장한 사이트 차단 목록은 그대로 유지됩니다.
...
| **차단 켜기**를 누를 수 없음 | 사이트·페이지 차단을 추가하거나 쇼츠·릴스 옵션을 선택한 뒤 **저장**하세요. 허용 규칙만으로는 차단을 켤 수 없습니다. |
| 등록한 사이트가 계속 열림 | 목록 저장 및 **차단 중** 상태와 겹치는 허용 규칙을 확인하고, 확장 프로그램 **세부정보 → 사이트 액세스 → 모든 사이트**로 설정하세요. |
| 사용 시간이 보이지 않음 | 기록이 일시정지되어 있지 않은지, 선택한 필터에 해당하는 사이트인지 확인하세요. 일반 Chrome 창에서 같은 사이트를 하루 누적 5분 초과 사용하면 표시됩니다. 설치·업데이트 이전의 방문 기록은 가져오지 않습니다. |
...
쇼츠·릴스 옵션을 선택하면 **차단 목록 사이트** 필터에 해당 서비스의 지원 호스트가 포함됩니다. 표시하는 시간은 일반 영상·게시물 사용까지 포함한 사이트 전체 시간이며, 쇼츠·릴스 시청 시간만 따로 구분하지 않습니다.

YouTube·Instagram 페이지에서는 콘텐츠 스크립트가 메뉴·추천 영역을 확인하여 숨깁니다. 페이지 내용은 저장하거나 외부로 전송하지 않습니다.

`storage`는 설정과 사용 시간 저장, `declarativeNetRequest`와 HTTP·HTTPS 사이트 접근 권한은 등록한 주소의 차단·허용과 사용 중인 사이트 확인에 사용합니다. `webNavigation`은 YouTube처럼 전체 페이지를 새로 불러오지 않고 이동하는 사이트에서도 규칙을 적용하는 데 사용합니다. `alarms`는 약 30초 간격의 사용 시간 확인, `idle`은 미사용·화면 잠금 상태를 확인하여 시간 기록을 멈추는 데 사용합니다. `favicon`은 팝업·사이트 관리·사용 시간 화면에서 Chrome이 제공하는 사이트 아이콘을 표시하는 데 사용합니다. 아이콘을 가져올 수 없으면 기본 아이콘으로 표시합니다.

## 디자인 미리보기

Node.js가 설치된 환경에서 프로젝트 최상위 폴더에서 실행하세요.

```sh
node dev/preview-server.mjs
```

[로컬 미리보기](http://127.0.0.1:4173)에서 사이트 관리·팝업·사용 시간·차단 화면을 전환하고 모바일 너비도 확인할 수 있습니다. 실제 화면 소스를 그대로 사용하며 HTML·CSS·JavaScript 파일을 저장하면 자동으로 새로고침됩니다.

미리보기의 파비콘은 샘플 사이트 6개의 로컬 이미지로 표시합니다. 실제 확장 프로그램은 Chrome 파비콘 기능을 사용합니다.

버튼과 저장은 **독립된 예시 데이터**로 동작합니다. 설치된 확장 프로그램의 설정이나 사용 기록은 바뀌지 않으며, 실제 웹사이트 차단은 실행하지 않습니다. 상단 **예시 데이터 초기화**로 처음 상태로 돌아갈 수 있습니다. 서버는 `Ctrl+C`로 종료합니다.
