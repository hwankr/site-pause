...
<img src="outputs/browser-check/rules-popup.png" alt="YouTube 차단 규칙 1개와 허용 예외 2개를 적용한 팝업 화면." width="340">
...
이미 설치했다면 파일을 업데이트한 뒤 `chrome://extensions`에서 **잠깐, 집중의 새로고침**을 누르세요. 기존 차단 목록은 유지됩니다.
...
차단·허용 목록과 켜짐·꺼짐 상태는 **현재 브라우저에만 저장**합니다. 외부 서버나 분석 도구를 사용하지 않으며, 방문 기록을 수집해 저장하지 않습니다.

`storage`는 설정 저장, `declarativeNetRequest`와 HTTP·HTTPS 사이트 접근 권한은 등록한 주소의 차단·허용에 사용합니다. `webNavigation`은 YouTube처럼 전체 페이지를 새로 불러오지 않고 이동하는 사이트에서도 규칙을 적용하는 데 사용합니다.
